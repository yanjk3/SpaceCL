...
import spacecl.loader
import spacecl.builder
import spacecl.transforms
import spacecl.datasets
import spacecl.models

model_names = sorted(name for name in spacecl.models.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(spacecl.models.__dict__[name]))
...
parser.add_argument('--weight-type', default=None)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = spacecl.builder.SpaceCL(spacecl.models.__dict__[args.arch],
    ...
    if args.aug_plus:
        ...
        flip_augmentation = spacecl.transforms.RandomHorizontalFlip()
        crop_augmentation = spacecl.transforms.RandomResizedCrop(224, scale=(0.2, 1.))
        ending_augmentation = transforms.Compose([
            transforms.RandomApply([spacecl.loader.GaussianBlur([.1, 2.])], p=0.5),
    ...
    train_dataset = spacecl.datasets.ImageFolder(
        traindir,
        args.img_size,
        args.weight_type,
        args.gamma,
        base_transform=base_augmentation,
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, label_weights, anchors, anchors_c3) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            ...
            label_weights = label_weights.cuda(args.gpu, non_blocking=True)
        ...
        # loss_moco = criterion(output, target, weight=label_weights)
        loss_moco = (F.cross_entropy(output, target, reduction='none')*label_weights).mean()
